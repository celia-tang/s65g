...
@IBDesignable class GridEditView: UIView {
    ...
    var points:[Position]! {
        get {
            return TableModel.sharedInstance.getLiving()
        }
        set {
            self.points = newValue
            TableModel.sharedInstance.gridSet(self.points)
        }
    }
    ...
    override func drawRect(rect: CGRect) {
        ...
        let cellWidth:CGFloat = (width - (CGFloat(TableModel.sharedInstance.cols + 1) * gridWidth)) / CGFloat(TableModel.sharedInstance.cols)
        let cellHeight:CGFloat = (height - (CGFloat(TableModel.sharedInstance.rows + 1) * gridWidth)) / CGFloat(TableModel.sharedInstance.rows)
        ...
        for i in 0...TableModel.sharedInstance.rows {
        ...
        for i in 0...TableModel.sharedInstance.cols {
        ...
        for i in 0..<TableModel.sharedInstance.rows {
            for j in 0..<TableModel.sharedInstance.cols {
                ...
                // set color
                switch TableModel.sharedInstance.grid[i, j] {
                case .Empty:
                    emptyColor.setFill()
                case .Alive:
                    aliveColor.setFill()
    ...
    func getCell(point: CGPoint) -> (x: Int ,y: Int){
        ...
        i >= 0 ? ((i <= width) ? m = Int(i / (width / CGFloat(TableModel.sharedInstance.cols))) : ()) : ()
        j >= 0 ? ((j <= height) ? n = Int(j / (height / CGFloat(TableModel.sharedInstance.rows))) : ()) : ()
        return (m, n)
    }
    ...
    func processTouch(touch: UITouch){
        ...
        if (cell.0 >= 0 && cell.1 >= 0 && cell.0 < TableModel.sharedInstance.cols && cell.1 < TableModel.sharedInstance.rows) {
            TableModel.sharedInstance.toggle(cell.1, j: cell.0)
